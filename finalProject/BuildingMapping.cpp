...
void BuildingMapping::clearScreen() {
    system("cls");
}

void BuildingMapping::pauseScreen() {
    cout << "\nPress Enter to continue...";
    cin.ignore(numeric_limits<streamsize>::max(), '\n');
    cin.get();
}

void BuildingMapping::printLine(int n) {
    cout << string(n, '=') << "\n";
}

void BuildingMapping::printTitle(const string& title) {
    printLine();
    cout << title << "\n";
    printLine();
}

string BuildingMapping::getRiskLevel(double score) {
...
string BuildingMapping::getRecommendation(double score) {
...
void BuildingMapping::showCountryMenu() {
...
void BuildingMapping::showRiskReport(Building* b, RiskModel* model, double score) {
...
void BuildingMapping::run() {
    ...
    while (true) {
        ...
        if (subChoice == 1) {
            ...
            delete model;
            return; // Goes back to main.cpp loop
        }
        else {
            cout << "\nInvalid option.\n";
            pauseScreen();
